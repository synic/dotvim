...
function M.goto_project_directory()
	local dir = config.options.projects.directory or "."
	vim.cmd.edit(dir)
end

function M.search_project_cursor_term()
...
local function dirpicker_pick_project(cb)
	...
	require("modules.picker").dir_picker(projects, "Projects", cb)
end
...
function M.new_tab_with_project()
...
function M.find_project_files(picker)
...
function M.find_project_files_smart()
	M.find_project_files("files")
end
